"""
Generates a dataset and latent variables based on Ahuja et al. (2023)
Can also be used for different types of data structures for us now only the scm_dense and scm_sparse cases
relevant
...
def generate_latent_vector(dataset_size, latent_dim, latent_case, intervention_indices=[], intervention_case=0,
                           dag=None, base_dir=''):
...
def execute():
    ...
    for distribution_case in ['observational', 'interventional']:
        for data_case in ['train', 'val', 'test']:
            ...
            f = base_dir + data_case + '.npy'
            y_and_x = np.concatenate((y[:, np.newaxis], x), axis=1)
            np.save(f, y_and_x)
