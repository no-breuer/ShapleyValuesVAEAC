...
def compute_normalization(data, one_hot_max_sizes):
    ...
    # iterate over the number of features and get the number of categories.
    for i, size in enumerate(one_hot_max_sizes):
        # Do not do anything if the feature is categorical
        if size >= 2:
            continue
        ...
        v = v[torch.logical_not(torch.isnan(v))]

        # Compute min and max of the observed values
        vmin, vmax = v.min(), v.max()

        # Compute the mean of the values
        vmean = v.mean()

        # Compute the std of the values
        vstd = v.std()

        # Save the mean and std in the right place of the vectors
